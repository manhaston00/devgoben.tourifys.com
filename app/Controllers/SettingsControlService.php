...
class SettingsControlService
{
    ...
    public function saveSection(string $sectionKey, string $scope, ?int $tenantId, ?int $branchId, array $input): array
    {
        ...
        foreach ($registry[$sectionKey]['settings'] as $setting) {
            ...
            $postedValue = $this->extractPostedValue($input, $key, $setting);

            if ($postedValue === null) {
                continue;
            }

            $posted   = $this->normalizePostedValue($setting, $postedValue);
            $resolved = $this->resolveSetting($setting, $appMap, $tenantMap, $branchMap);
    ...
    protected function canPersistSettingForScope(array $setting, string $scope): bool
    {
        $settingScope = (string) ($setting['scope'] ?? 'tenant');
    ...
    protected function extractPostedValue(array $input, string $settingKey, array $setting)
    {
        $inputName = $this->inputName($settingKey);
